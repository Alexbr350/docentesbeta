    ...
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 transition-colors">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 grid grid-cols-3 gap-5">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-md">
            ...
            {amigos.map((amigo) => (
              ...
              >
                ...
                <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">{amigo.nombre}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="col-span-2">
          ...
          {amigoSeleccionado && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-md flex flex-col h-[500px]">
              <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2">
                <div className="w-9 h-9 rounded-xl bg-blue-600 text-white flex items-center justify-center text-sm font-bold">
                  {amigoSeleccionado.nombre?.charAt(0).toUpperCase()}
                </div>
                <p className="text-sm font-bold text-gray-800 dark:text-slate-100">{amigoSeleccionado.nombre}</p>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-2">
                ...
                {mensajes.map((m) => (
                  <div key={m.id} className={`flex ${m.de === user?.email ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[70%] px-3 py-2 rounded-xl text-sm ${m.de === user?.email ? "bg-blue-600 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300"}`}>
                      {m.texto}
                    </div>
                  </div>
                ))}
              ...
              <div className="p-3 border-t border-slate-100 dark:border-slate-800 flex gap-2">
                <input
                  ...
                  className="flex-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-blue-400"
                />
