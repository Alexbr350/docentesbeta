  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <h3 className="text-sm font-extrabold text-slate-500 uppercase tracking-widest mb-3">Próximos eventos</h3>
        {proximos.length === 0 && (
          <div className="bg-white rounded-2xl p-8 text-center text-slate-400 text-sm shadow-md mb-6">
            No hay eventos próximos por el momento.
          </div>
        )}
        {proximos.map((e) => (
          <div key={e.id} className="bg-white rounded-2xl p-5 mb-3 shadow-md hover:shadow-lg transition">
            {e.imagenUrl && /\.pdf$/i.test(e.imagenNombre || "") ? (
              ...
              <img src={e.imagenUrl} alt={e.titulo} className="w-full rounded-xl mb-3 border border-slate-200 max-h-64 object-cover" />
            )}
            <p className="text-sm font-extrabold text-gray-800">{e.titulo}</p>
            <p className="text-sm text-slate-600 mt-1 leading-relaxed">{e.descripcion}</p>
        ...
        {pasados.length > 0 && (
          <>
            <h3 className="text-sm font-extrabold text-slate-500 uppercase tracking-widest mb-3 mt-8">Eventos pasados</h3>
            {pasados.map((e) => (
              <div key={e.id} className="bg-white rounded-2xl p-5 mb-3 shadow-md opacity-60">
                <p className="text-sm font-extrabold text-gray-800">{e.titulo}</p>
                <p className="text-sm text-slate-600 mt-1 leading-relaxed">{e.descripcion}</p>
